...
Java_com_chesslab_engine_Stockfish_nativeStart(JNIEnv *env, jobject, jstring path) {
...
JNIEXPORT void JNICALL
Java_com_chesslab_engine_Stockfish_nativeSend(JNIEnv *env, jobject, jstring command) {
    const char *nativeCommand = env->GetStringUTFChars(command, nullptr);
    cstockfish_send(nativeCommand);
...
Java_com_chesslab_engine_Stockfish_nativeReadLine(JNIEnv *env, jobject) {
    std::string line;
    {
...
JNIEXPORT void JNICALL
Java_com_chesslab_engine_Stockfish_nativeStop(JNIEnv *, jobject) {
    cstockfish_stop();
    {
        std::lock_guard<std::mutex> lock(gMutex);
        gClosed = true;
...
Java_com_chesslab_engine_Stockfish_nativeIsRunning(JNIEnv *, jobject) {
    return cstockfish_is_running() ? JNI_TRUE : JNI_FALSE;
}

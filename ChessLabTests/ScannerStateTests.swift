...
struct ScannerStateTests {
    ...
    @Test func goingBackToTheSourceForgetsThePreviousReading() throws {
        let vm = ScannerViewModel()
        let image = try #require(boardImage())
        vm.load(UIImage(cgImage: image))
    ...
    @Test func aFailedAutomaticCropFallsBackToManualAdjustment() throws {
        let vm = ScannerViewModel()
        let image = try #require(boardImage())
        vm.load(UIImage(cgImage: image))
    ...
    @Test func goingBackToTheCropClearsTheError() throws {
        let vm = ScannerViewModel()
        let image = try #require(boardImage())
        vm.load(UIImage(cgImage: image))

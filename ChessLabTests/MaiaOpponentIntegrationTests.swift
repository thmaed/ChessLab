...
/// Core ML et le VRAI Stockfish (filet) : Camille joue les Blancs, répond au
...
struct MaiaOpponentIntegrationTests {
    ...
    @Test("Camille ouvre la partie et répond, sans que le filet se déclare indisponible")
    func camillePlaysBothHerMoves() async throws {
        try await EngineIntegrationGate.shared.withExclusiveAccess {
            ...
            settings.opponentProfileID = OpponentProfile.camille.id
            ...
            #expect(vm.opponentProfile == .camille)
            #expect(vm.opponentDisplayName == "Camille")
            ...
            try #require(!vm.moveLog.isEmpty, "Camille n'a jamais joué son premier coup")
            ...
            // Réponse du joueur, puis second coup de Camille.
            ...
            #expect(vm.moveLog.count == 3, "Camille n'a pas répondu")

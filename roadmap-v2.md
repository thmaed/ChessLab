...
- 09/08/2026 — **Refonte de l'intégration Stockfish** (perf + freezes) :
  ...
  - **Réglages appareil (09/08, fait)** : `DevicePerformance` — threads = cœurs PERFORMANCE (`hw.perflevel0`, plafonné 4) au lieu de `cœurs − 2` (iPhone 11 : 2 au lieu de 4) ; budget de classification et profondeur live PALIÉRISÉS par RAM (bas 220k/prof.20, milieu 380k/24, haut 550k/26) → analyse plus profonde sur matériel moderne, sobre en bas de gamme. Testé sur **iPhone 16 (rapide)** ; le cas **iPhone 11 (A13, reporté par un utilisateur) reste à valider sur appareil** (TestFlight au reporter conseillé — le simulateur ne reproduit pas le CPU A13).
  - **Reste possible (non fait)** : mesurer le gain NPS réel sur appareil ; dotprod avec dispatch runtime (surplus A13+). Le « petit net » est écarté (pas d'option SF17 propre, inutile avec un seul moteur/net par process).

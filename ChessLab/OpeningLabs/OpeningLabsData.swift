...
/// Données SIDECAR du module « Ouvertures — Labs » : pour chaque position d'un
/// cours, ce que jouent les MAÎTRES et ce que dit STOCKFISH.
///
...
///    possible à un module qui marche, pour un module en aperçu.
/// 2. **Ce n'est pas la même donnée.** ``MoveEdge/gamesMasters`` ne décrit que
///    les arêtes CURÉES du graphe ; Labs veut tous les coups de maîtres de la
///    position, y compris ceux que le cours ne retient pas.
/// 3. **Coût.** Le sidecar ne se charge que si l'aperçu est activé, et
///    seulement pour l'ouverture ouverte. Qui n'allume pas Labs ne paie rien.
///
...
struct LabsMasterMove: Codable, Hashable, Sendable, Identifiable {
    let san: String
    let uci: String
...
struct LabsMasterStats: Codable, Hashable, Sendable {
    private var white_: Int?
    private var draws_: Int?
    private var black_: Int?
    private var moves_: [LabsMasterMove]?
    ...
    var draws: Int { draws_ ?? 0 }
    var blackWins: Int { black_ ?? 0 }
    var moves: [LabsMasterMove] { moves_ ?? [] }
    ...
    func share(of move: LabsMasterMove) -> Double {
        guard totalGames > 0 else { return 0 }
        return Double(move.games) / Double(totalGames)
    }

    init(white: Int, draws: Int, black: Int, moves: [LabsMasterMove]) {
        self.white_ = white
        self.draws_ = draws
        self.black_ = black
        self.moves_ = moves
...
struct LabsEngineLine: Codable, Hashable, Sendable, Identifiable {
    let san: String
    let uci: String
...
struct LabsPositionData: Codable, Hashable, Sendable {
    var masters: LabsMasterStats?
    private var engine_: [LabsEngineLine]?
    ...
    var engine: [LabsEngineLine] { Array((engine_ ?? []).prefix(3)) }

    var isEmpty: Bool { masters == nil && engine.isEmpty }

    init(masters: LabsMasterStats? = nil, engine: [LabsEngineLine] = []) {
...
struct OpeningLabsSidecar: Codable, Hashable, Sendable {
    ...
    var engineDepth: Int?
    let positions: [String: LabsPositionData]

    func data(at fen: String) -> LabsPositionData? { positions[fen] }

    init(schemaVersion: Int? = OpeningLabsSchema.currentVersion, id: String, engineDepth: Int? = nil, positions: [String: LabsPositionData]) {
...
/// Charge les sidecars Labs — un fichier par cours, à la demande.
///
...
enum OpeningLabsLoader {
    ...
    private static var cached: (id: String, sidecar: OpeningLabsSidecar)?

    /// Sous-dossier de ressources — même convention que `openings/`.
    static let subdirectory = "openings_labs"

    static func sidecar(id: String, in bundle: Bundle = .main) -> OpeningLabsSidecar {
        if let cached, cached.id == id { return cached.sidecar }
        let loaded = load(id: id, in: bundle) ?? OpeningLabsSidecar(id: id, positions: [:])
        cached = (id, loaded)
        return loaded
    }

    /// Vide le cache mémoire (tests, et bascule de l'aperçu dans les réglages).
    static func flush() { cached = nil }

    private static func load(id: String, in bundle: Bundle) -> OpeningLabsSidecar? {
        guard
            let url = url(id: id, in: bundle),
            let data = try? Data(contentsOf: url),
            let sidecar = try? JSONDecoder().decode(OpeningLabsSidecar.self, from: data)
        else {
            return nil
        }
        return sidecar
    ...
    private static func url(id: String, in bundle: Bundle) -> URL? {
        // Le nom porte « .labs » pour ne pas entrer en collision avec le cours
        // du même identifiant si un dossier de ressources se retrouvait aplati
        // au moment de la copie (c'est déjà arrivé, cf. `OpeningCourseLoader`).
        bundle.url(forResource: "\(id).labs", withExtension: "json", subdirectory: subdirectory)
            ?? bundle.url(forResource: "\(id).labs", withExtension: "json")
    }

    /// Décode depuis des octets bruts — pour les tests, sans dépendance au bundle.
    nonisolated static func decode(from data: Data) throws -> OpeningLabsSidecar {
        try JSONDecoder().decode(OpeningLabsSidecar.self, from: data)
    }
}

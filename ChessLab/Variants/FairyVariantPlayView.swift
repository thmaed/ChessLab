...
/// Écran de partie dans une variante Fairy-Stockfish — la structure de
/// ``Chess960PlayView`` (plateau au centre, rangées joueurs, barre de
/// contrôle unique, menu d'export, abandon), avec la teinte et l'icône de
/// la variante en identité visuelle.
struct FairyVariantPlayView: View {
    ...
    private func playerRow(for color: Piece.Color) -> some View {
        HStack(spacing: 10) {
            ...
            // un doublon confus plutôt qu'utile.
            ...
            // Trois échecs : le compteur de chaque camp, visible en permanence
            // — c'est la ressource qui décide la partie, comme le temps
            // restant sur une pendule.
            if variant.id == FairyVariant.threeCheck.id {
                checkCountBadge(for: color)
            }
            Spacer()
    ...
    private func checkCountBadge(for color: Piece.Color) -> some View {
        HStack(spacing: 3) {
            Image(systemName: "bolt.fill").font(.caption2)
            Text("\(viewModel.checkCounts[color] ?? 0)/3")
        ...
        .accessibilityLabel(Text("\(viewModel.checkCounts[color] ?? 0) échecs sur 3"))
    }
    ...
    /// Marqueur invisible pour les tests d'interface — la RAISON de fin de
    /// partie, qui prouve que la condition PROPRE à la variante (pas
    /// seulement mat/pat) a bien été détectée.

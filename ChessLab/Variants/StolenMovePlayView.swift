...
/// Écran de partie en Coup Volé — même structure que
/// ``FairyVariantPlayView``, avec l'affichage du jeton et du tour double en
/// plus (badge jeton par rangée joueur, bandeau « second coup » pendant un
/// tour double).
struct StolenMovePlayView: View {
    ...
    /// posées — bandeaux joueurs, barre de contrôle, bande des coups, et les
    /// panneaux qui surgissent en cours de partie (offre de jeton, fin de
    /// partie). Ce lecteur de géométrie mesure l'espace réellement laissé par
    /// la pile, au lieu de parier une fraction de la fenêtre (`hauteur × 0,62`) :
    /// ce pari ignorait les rangées et tronquait le plateau dès que la fenêtre
    /// devenait courte — sur Mac, dès sa taille minimale. Il rend aussi les
    /// grandes tailles de texte gratuites : les rangées grandissent, le
    /// plateau se réduit d'autant, sans facteur dédié.
    ...
    private func playerRow(for color: Piece.Color) -> some View {
        HStack(spacing: 10) {
            ...
            if (viewModel.tokens[color] ?? 0) > 0 {
                tokenBadge
            }
            if viewModel.awaitingSecondMoveBy == color {
                secondMoveBadge
            }
            Spacer()
    ...
    private var tokenBadge: some View {
        Image(systemName: "bolt.badge.clock.fill")
            ...
            .padding(4)
            .background(variant.tint.opacity(0.16), in: Circle())
            .accessibilityLabel("Jeton disponible")
    }

    private var secondMoveBadge: some View {
        Text("2ᵉ coup")
            .font(.caption2.weight(.bold))
            .foregroundStyle(variant.tint)
            .padding(.horizontal, 6)
            .padding(.vertical, 2)
            .background(variant.tint.opacity(0.16), in: Capsule())
            .accessibilityLabel("En train de jouer son second coup")
    }

    /// Bandeau proposant de dépenser le jeton — visible seulement quand
    /// l'utilisateur peut vraiment le faire (son tour, hors échec, premier
    /// coup du tour). Un jeton dépensé ne se voit qu'après coup (badge « 2e
    /// coup » ci-dessus) : ce bandeau, lui, est l'INVITATION à le dépenser
    /// AVANT de jouer.
    @ViewBuilder
    private var tokenOfferBanner: some View {
        if viewModel.canOfferTokenNow {
            HStack(spacing: 10) {
                Image(systemName: "bolt.badge.clock.fill")
                    .foregroundStyle(variant.tint)
                Text(viewModel.wantsToSpendToken
                     ? "Le prochain coup dépensera votre jeton — deux coups d'affilée."
                     : "Vous avez un jeton : dépensez-le pour jouer deux coups d'affilée.")
                    .font(.footnote)
                    .foregroundStyle(Theme.textPrimary)
                Spacer(minLength: 0)
                Toggle("", isOn: $viewModel.wantsToSpendToken)
                    .labelsHidden()
                    .tint(variant.tint)
                    .accessibilityLabel("Dépenser le jeton sur le prochain coup")
            }
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .background(variant.tint.opacity(0.12), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
        }
    }

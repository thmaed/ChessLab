...
/// Revue d'une partie de Duck Chess terminée — même anatomie que
/// ``VariantAnalysisView`` (plateau, barre d'éval, flèche du meilleur coup,
/// liste des coups cliquable, navigation), avec le canard à sa place à
/// chaque demi-coup : sans lui, la moitié de la partie serait illisible.
struct DuckChessAnalysisView: View {
    let viewModel: DuckChessAnalysisViewModel
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            .padding(.vertical, 8)
            // Colonne bornée par la HAUTEUR : sans elle, une fenêtre Mac
            // large étirerait la barre de navigation et la liste des coups
            // sur toute sa largeur, loin du plateau. Même mesure que les
            // écrans de partie.
        ...
        .appBackground()
        .navigationTitle(Text(LocalizationController.string("Analyse") + " — " + variant.displayName))
    ...
    private func boardBlock(size: CGSize) -> some View {
        ...
        return VStack(spacing: 8) {
            ...
            ChessBoardView(
                ...
                // Ni échec, ni mat en Duck Chess : un anneau rouge autour du
                // roi mentirait sur la règle.

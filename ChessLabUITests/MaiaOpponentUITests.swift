...
final class MaiaOpponentUITests: XCTestCase {
    ...
    func testCharacterModeShowsCamilleAndStartsAGame() throws {
        ...
        let card = app.descendants(matching: .any).matching(identifier: "opponentCard_camille").firstMatch
        XCTAssertTrue(card.waitForExistence(timeout: 3), "la carte de Camille doit apparaître")
        XCTAssertTrue(card.label.contains("Camille"))
        // La galerie : choisir Théo, la carte suit.
        let theo = app.descendants(matching: .any).matching(identifier: "opponentTile_theo").firstMatch
        XCTAssertTrue(theo.waitForExistence(timeout: 3), "la vignette de Théo doit exister")
        var tries = 0
        ...
        XCTAssertTrue(theoCard.waitForExistence(timeout: 3), "la carte de Théo doit apparaître")
        ...
        // La suite du parcours se joue contre Théo : son prénom doit
        ...
        XCTAssertTrue(app.staticTexts["Théo"].waitForExistence(timeout: 5), "le prénom remplace « Ordinateur »")
        ...
        XCTAssertTrue(app.staticTexts["Théo"].exists)
